...
void CExamDoc::Serialize(CArchive& ar)
{
	...
	else 
	{
		...
		if (infile->GetLength() != m_SizeX * m_SizeY) { // 파일 사이즈의 크기를 검사함
			AfxMessageBox(_T("파일의 크기가 안맞음."));
			return;
		}
		ar.Read(m_InImg[0], infile->GetLength()); //영상파일을 읽어 m_InImg에 저장함.
	}
...
void CExamDoc::m_Zoom(int height, int width, unsigned char InImg[][256])
{
	...
	{
		input_x = m_ZoomDlg.m_zoomsize_x;
		input_y = m_ZoomDlg.m_zoomsize_y;
	}

	BYTE* pZoomImg;
	BYTE newValue;

	int new_height = (int)(height * input_y); //새로운 이미지의 높이 계산 
	int new_width = (int)(width * input_x); //새로운 이미지의 폭 계산 
	...
	if (input_x > 1 && input_y > 1 ) { temp_height = height; temp_width = width; }  //둘다 확대인 경우
	else if(input_x > 1 && input_y < 1) { temp_height = new_height; temp_width = width; } //x만 확대
	else if (input_x < 1 && input_y > 1) { temp_height = height; temp_width = new_width; } //y만 확대
